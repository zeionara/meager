#ifndef EVALUATION_CLASSIFICATION_EVALUATOR_H
#define EVALUATION_CLASSIFICATION_EVALUATOR_H

#include <functional>

#include "../corruption/CorruptionStrategy.h"
#include "../samplers/DoubleBatch.h"
...
struct ClassificationEvaluator {
    ...
    ClassificationEvaluator(Corpus* corpus, ThickTripleListWrapper<T>* triples, ThinTripleListWrapper<T>* testTriples) {
        this->corpus = corpus;
        this->triples = triples;
        this->testTriples = testTriples;
    }

    DoubleBatch* makeBatch() { // Generate all possible triples for every entity which may be used as a triple head
        Triple* corruptedTriples = (Triple*) malloc(testTriples->content->length * sizeof(Triple));

        for (INT i = 0; i < testTriples->content->length; i++) {
            corruptedTriples[i] = corruptionStrategy->corrupt(testTriples->content->items[i]);
        }

        return new DoubleBatch(
            new TripleBatch(testTriples->content, testTriples->content->length),  // positive
            new TripleBatch(corruptedTriples, testTriples->content->length)  // negative
        );
    }
